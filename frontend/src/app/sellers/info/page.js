                ...
                {comments.map((comment) => (
                  <div key={comment.id} className="flex justify-end">
                    <div className="flex-grow text-right">
                      ...
                      <div className={`inline-block max-w-[80%] rounded-lg px-4 py-2 ${comment.isCustomer ? 'bg-purple-500 text-white' : 'bg-gray-100 dark:bg-gray-700 dark:text-white'}`}>
                        {comment.content}
                      </div>
                    </div>
                    <div className="flex-shrink-0 ml-2">
                      <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-600 flex items-center justify-center">
                        <span className="text-sm">{comment.author ? comment.author.charAt(0) : '?'}</span>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              <div className="border-t dark:border-gray-700 pt-4">
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={async () => {
                      if (commentText.trim()) {
                        console.log(`Estimate ID: ${selectedQuoteForComment.id}`);
                        console.log(`Author ID: ${sellerInfo.id}`);
